...
# Conditions initiales T1 -------------------------------------------------------------------------------------
time    = 0                                         # s
phi     = 0/180*np.pi                               # angle de rotation
rayon   = Lx/6                                      # défini le rayon (taille) // petit diviseur = grand rayon
x0      = Lx/5                                      # défini position en x
y0      = -Ly/20                                    # défini position en y
a       = rayon/1                                   # défini la déformation de la longueur 
b       = a/7                                       # défini la déformation de la largeur
T       += deltaT * np.exp(-((x-x0)*np.cos(phi) + (y-y0)*np.sin(phi))**2/a**2 - (-(y-y0)*np.cos(phi) + (x-x0)*np.sin(phi))**2/b**2) # gaussien en forme d'une ellipse tournée // formule dans matlab #T =  (deltaT * np.exp((-x**2/wn_x - (y/wn_y + np.max(y/wn_y)/2)**2)/w**2))      # np.exp pour exponentielle ajouter T+= ........ pour ajouter des forme a ma T                                    # défini Pf comme une matric de 0 de taille T()

# Conditions initiales T2 --------------------------------------------------------------------------------------
phi2    = 50/180*np.pi                              # angle de rotation
rayon2  = Lx/6                                      # défini le rayon (taille) // petit diviseur = grand rayon
x02     = -Lx/5                                     
y02     = Ly/4                                      # défini position en y
a2      = Lx/4                                      # défini la déformation de la longueur
b2      = a2/5                                      # défini la déformation de la largeur
T      += deltaT * np.exp(-((x-x02)*np.cos(phi2) + (y-y02)*np.sin(phi2))**2/a2**2 - (-(y-y02)*np.cos(phi2) + (x-x02)*np.sin(phi2))**2/b2**2) # gaussien en forme d'une ellipse tournée // formule dans matlab #T =  (deltaT * np.exp((-x**2/wn_x - (y/wn_y + np.max(y/wn_y)/2)**2)/w**2))      # np.exp pour exponentielle ajouter T+= ........ pour ajouter des forme a ma T                                   # défini Pf comme une matric de 0 de taille T()

# Conditions initiales T3 ---------------------------------------------------------------------------------------
phi3    = -70/180*np.pi                             # angle de rotation
rayon3  = Lx/6                                      # défini le rayon (taille) // petit diviseur = grand rayon
x03     = -Lx/11
y03     = -Ly/4                                     # défini position en y
a3      = Lx/8
b3      = a3/5                                      # défini la déformation de la largeur
T      += deltaT * np.exp(-((x-x03)*np.cos(phi3) + (y-y03)*np.sin(phi3))**2/a3**2 - (-(y-y03)*np.cos(phi3) + (x-x03)*np.sin(phi3))**2/b3**2) # gaussien en forme d'une ellipse tournée // formule dans matlab #T =  (deltaT * np.exp((-x**2/wn_x - (y/wn_y + np.max(y/wn_y)/2)**2)/w**2))      # np.exp pour exponentielle ajouter T+= ........ pour ajouter des forme a ma T

# Conditions initiales T4 ----------------------------------------------------------------------------------------
phi4    = 90/180*np.pi                              # angle de rotation
rayon4  = Lx/6                                      # défini le rayon (taille) // petit diviseur = grand rayon
x04     = Lx/3
y04     = -Ly/2                                     # défini position en y
a4      = Lx/12
b4      = a4/1                                      # défini la déformation de la largeur
T      += deltaT * np.exp(-((x-x04)*np.cos(phi4) + (y-y04)*np.sin(phi4))**2/a4**2 - (-(y-y04)*np.cos(phi4) + (x-x04)*np.sin(phi4))**2/b4**2) # gaussien en forme d'une ellipse tournée // formule dans matlab #T =  (deltaT * np.exp((-x**2/wn_x - (y/wn_y + np.max(y/wn_y)/2)**2)/w**2))      # np.exp pour exponentielle ajouter T+= ........ pour ajouter des forme a ma T
...
for it in arr:
    ...
    # plot T normal ================================================================================================
        ...
        plt.savefig('./B_img/B_img_'+it_long+'.png')

...
Pr_char     = lam_rhoCp/k_etaf                      # (m^2/s)/(m^2/(Pa*s)) = Pa, pression charactéristique
...
nout    = 20                                        # Plot every nout time step
st      = 3                                         # Plot every st velocity, nombre de flèche
niter   = 1000                                   # défini la taille de ma boucle inter
...
time    = 0                                         # s
phi     = 0/180*np.pi                               # angle de rotation
rayon   = Lx/6                                      # défini le rayon (taille) // petit diviseur = grand rayon
x0      = 0                                         # défini position en x
y0      = -Ly/2                                     # défini position en y
a       = rayon/1                                   # défini la déformation de la longueur 
b       = a/3                                       # défini la déformation de la largeur
T       = 0+(y-Ly/2)/Ly*-deltaT/2
T      += np.zeros((nx,ny)) #deltaT * np.exp(-((x-x0)*np.cos(phi) + (y-y0)*np.sin(phi))**2/a**2 - (-(y-y0)*np.cos(phi) + (x-x0)*np.sin(phi))**2/b**2) # gaussien en forme d'une ellipse tournée // formule dans matlab #T =  (deltaT * np.exp((-x**2/wn_x - (y/wn_y + np.max(y/wn_y)/2)**2)/w**2))      # np.exp pour exponentielle ajouter T+= ........ pour ajouter des forme a ma T                                    # défini Pf comme une matric de 0 de taille T()
INSIDE  = ((x-x0)*np.cos(phi) + (y-y0)*np.sin(phi))**2/a**2 + (-(y-y0)*np.cos(phi) + (x-x0)*np.sin(phi))**2/b**2 < 1 # ellipse tourné
T[INSIDE] = deltaT*0.5

T[:, 0]   = T[:, 1]                            # heating at the base by deltaT/2
T[:, -1]  = T[:, -2]
...
# adding 2 wells ==================================================================================================
# injection well 1
x_well1 = -Lx/6     # x-position
y_well1 = -0.155         # y-position
q_well1 = 1e-3      # injection Darcy velocity
T_well1 = 10 # injection Temperature (needs to be defined, is not calculated!, otherwise is zero)

# out-pumping well 2
x_well2 = Lx/9      # x-position
y_well2 = -0.155         # y-position
q_well2 = -q_well1  # out-pumping Darcy velocity
# T_well2 -> extraction Temperature is calculated ;)

#  deriving well x,y-indices
xid_well1 = np.floor((x_well1 + Lx/2)/Lx*nx).astype(int)
yid_well1 = np.floor((y_well1 + Ly/3)/Ly*ny).astype(int)
xid_well2 = np.floor((x_well2 + Lx/2)/Lx*nx).astype(int)
yid_well2 = np.floor((y_well2 + Ly/3)/Ly*ny).astype(int)
T[xid_well1,yid_well1] = T_well1
...
for it in arr:
    ...
    for inter in range(1,niter):
        ...
        # well 1
        qx[xid_well1  ,yid_well1  ] = -q_well1/4 # left
        qx[xid_well1+1,yid_well1  ] =  q_well1/4 # right      
        qy[xid_well1  ,yid_well1  ] = -q_well1/4 # bottom
        qy[xid_well1  ,yid_well1+1] =  q_well1/4 # top   
        
        # well 2
        qx[xid_well2  ,yid_well2  ] = -q_well2/4 # left
        qx[xid_well2+1,yid_well2  ] =  q_well2/4 # right     
        qy[xid_well2  ,yid_well2  ] = -q_well2/4 # bottom
        qy[xid_well2  ,yid_well2+1] =  q_well2/4 # top
    ...
    # plot T normal ================================================================================================
        ...
        """xt = [(x_well1-6.5)/Ly,(x_well1-6.5)/Ly] 
        yt = [y_well1,0.5]"""
        
        xt = [x[xid_well1,1]/Ly,x[xid_well1,1]/Ly] 
        yt = [y[1,yid_well1]/Ly,y[1,-1]/Ly]

        ax.plot(xt,yt,color='black',linewidth=3)
        
        xt2 = [x[xid_well2,1]/Ly,x[xid_well2,1]/Ly] 
        yt2 = [y[1,yid_well2]/Ly,y[1,-1]/Ly]

        ax.plot(xt2,yt2,color='black',linewidth=3)
        
        ax.quiver(x_quiver, y_quiver, qx_quivers, qy_quivers)
        ax.set_title('it = ' + str(int(it)) + '   temps = ' + str(int(dt/3600*it/24)) + ' jours    T° in ' + str(round(T[xid_well2,yid_well2],2)) + ' C°    T° out ' + str(int(T[xid_well1,yid_well1])) + ' C°')
        ...
        plt.savefig('./D_img/D_img_'+it_long+'.png')
